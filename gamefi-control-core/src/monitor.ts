...
import { GameFiSourceConfig, SourceHealthPayload } from './types';
import { createLogger } from './utils/logger';

const logger = createLogger('gamefi-control-core');
 ...
 *
 * This service is config-driven. It reads state from the local gamefi-source.json
 * and emits what the config says. It does NOT query the deployed InboundReceiver
 * contract directly. On-chain source management is handled by the legacy-named:
 ...
 * update config/gamefi-source.json to reflect the real addresses.
 */
export class GameFiSourceMonitor {
  private kafka: Kafka;
  private producer: Producer;
  private serviceConfig: ReturnType<typeof getServiceConfig>;
  private sourceConfig: GameFiSourceConfig | null = null;
  ...
  async start(): Promise<void> {
    ...
    if (errors.length > 0) {
      throw new Error(
        `Invalid GameFi source config (${this.serviceConfig.sourceConfigPath}):\n` +
          errors.map((e) => `  - ${e}`).join('\n'),
      );
    }
  ...
  async emitHealthSnapshot(): Promise<void> {
    ...
    await this.producer.send({
      ...
      messages: [{ key: 'gamefi-source-health', value: JSON.stringify(payload) }],
    });

    logger.info({ state, topic: this.serviceConfig.outputTopic }, 'GameFi source health emitted');
  }
  ...
  async reload(): Promise<void> {
    ...
    const state = deriveState(this.sourceConfig);
    logger.info({ state }, 'GameFi source config reloaded');
    await this.emitHealthSnapshot();
  }
  ...
  async stop(): Promise<void> {
    ...
    logger.info('GameFi source monitor stopped');
  }
