...
const azureFunction: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    // ── Deduplication: skip leads already processed in the last 24h ──
    const cutoff = new Date(Date.now() - DEDUP_WINDOW_MS).toISOString();
    let existingIds = new Set<string>();
    try {
        const { resources } = await container.items
            .query({
                query: "SELECT c.daoId FROM c WHERE c.timestamp >= @cutoff",
                parameters: [{ name: "@cutoff", value: cutoff }]
            })
            .fetchAll();
        existingIds = new Set(resources.map((r: any) => r.daoId));
    } catch (err: any) {
        context.log(`[Hepar Lead Scan] Dedup query failed — proceeding without dedup: ${err.message}`);
    }
