...
function defaultForensics(input?: Partial<HeparForensicsSignals>): HeparForensicsSignals {
  return {
    bytecode: {
...
function adminKeyRisk(model: HeparForensicsSignals['adminKeyModel']): number {
  switch (model) {
    case 'renounced':
      return 0;
...
function upgradeabilityRisk(model: HeparForensicsSignals['upgradeability']): number {
  switch (model) {
    case 'immutable':
      return 0;
    case 'timelocked':
...
function bytecodeRisk(f: HeparForensicsSignals): number {
  return clamp100(
    (normalize(f.bytecode.uncheckedOwnerDrainFunctions, 4) * 100 * 0.30) +
...
function proxyControlRisk(f: HeparForensicsSignals): number {
  const proxyPatternRisk =
    f.proxyAdmin.proxyPattern === 'beacon'
      ? 65
  ...
  return clamp100(
    ((f.proxyAdmin.proxyAdminPresent ? 1 : 0) * 100 * 0.10) +
...
function lpUnlockRisk(f: HeparForensicsSignals): number {
  return clamp100(
    (normalize(Math.max(0, f.lpUnlock.lpTopHolderConcentrationPct - 45), 55) * 100 * 0.35) +
...
function walletTaintRisk(f: HeparForensicsSignals): number {
  return clamp100(
    (normalize(f.walletTaint.taintedFundingLinks, 5) * 100 * 0.30) +
...
function adversarialRisk(f: HeparForensicsSignals): number {
  return clamp100(
    (normalize(f.adversarial.maxIntPathVulnerabilities, 5) * 100 * 0.25) +
...
function computeRiskBreakdown(_opportunity: HeparOpportunity, f: HeparForensicsSignals): HeparRiskBreakdown {
  ...
  const toxicityRisk = clamp100(
    (f.honeypotProbability * 100 * 0.30) +
  ...
  const governanceControlRisk = clamp100(
    (adminKeyRisk(f.adminKeyModel) * 0.25) +
      (upgradeabilityRisk(f.upgradeability) * 0.15) +
      (normalize(f.privilegedFunctionCount, 15) * 100 * 0.10) +
      (proxyLayerRisk * 0.50),
  );

  const liquidityExitRisk = clamp100(
    (normalize(Math.max(0, 70 - f.liquidityLockPercent), 70) * 100 * 0.20) +
      (normalize(Math.max(0, 30 - f.liquidityLockDays), 30) * 100 * 0.20) +
      (normalize(Math.max(0, f.top10HolderConcentrationPct - 45), 55) * 100 * 0.20) +
      (lpLayerRisk * 0.40),
  );

  const marketIntegrityRisk = clamp100(
    (f.washTradingScore * 0.20) +
  ...
  const dependencyContagionRisk = clamp100(
    (f.dependencyRiskScore * 0.65) +
...
function hardBlocks(opportunity: HeparOpportunity, f: HeparForensicsSignals, risk: HeparRiskBreakdown): string[] {
  const blocks: string[] = [];

  if (opportunity.opaque) blocks.push('opaque_structure');
  if (opportunity.exploitative) blocks.push('exploitative_posture');
  if (opportunity.counterpartyRisk === 'high') blocks.push('counterparty_risk_high');
  if (opportunity.structuralRisk === 'high') blocks.push('structural_risk_high');
  if (opportunity.liquidityScore < 50) blocks.push('liquidity_quality_below_floor');
  if (opportunity.edgeBps < 12) blocks.push('edge_below_floor');

  if (f.honeypotProbability >= 0.6) blocks.push('honeypot_probability_excessive');
  if (f.criticalAuditFindings >= 2) blocks.push('critical_audit_findings_unresolved');
  if (f.unresolvedIncidents30d > 0) blocks.push('recent_unresolved_incidents');

  if (f.bytecode.uncheckedOwnerDrainFunctions > 0) blocks.push('unchecked_owner_drain_functions');
  if (f.bytecode.selfdestructReachable) blocks.push('selfdestruct_reachable');
  if (f.bytecode.uninitializedProxyImplementation) blocks.push('uninitialized_proxy_implementation');

  if (
    f.proxyAdmin.upgradeAuthorityAddressCount <= 1 &&
  ...
  if (
    f.lpUnlock.lpTopHolderConcentrationPct >= 70 &&
  ...
  if (risk.aggregateRisk >= 75) blocks.push('aggregate_risk_excessive');

  return blocks;
}

function scoreOpportunity(opportunity: HeparOpportunity, f: HeparForensicsSignals, risk: HeparRiskBreakdown): number {
  ...
  if (opportunity.opaque) score -= 40;
  if (opportunity.exploitative) score -= 50;
...
function inferConfidence(opportunity: HeparOpportunity, f: HeparForensicsSignals): 'low' | 'medium' | 'high' {
  let observedSignals = 0;
  if (opportunity.forensics?.bytecode) observedSignals += 1;
  if (opportunity.forensics?.proxyAdmin) observedSignals += 1;
  if (opportunity.forensics?.lpUnlock) observedSignals += 1;
  if (opportunity.forensics?.walletTaint) observedSignals += 1;
  if (opportunity.forensics?.adversarial) observedSignals += 1;

  if (observedSignals >= 5 && f.contractAgeDays >= 30) return 'high';
  if (observedSignals >= 3) return 'medium';
  return 'low';
}

function reasonCodes(
  opportunity: HeparOpportunity,
  f: HeparForensicsSignals,
  risk: HeparRiskBreakdown,
  hardBlockReasons: string[],
  approved: boolean,
): string[] {
  const reasons: string[] = [];

  if (opportunity.opaque) reasons.push('opaque structure rejects safe metabolism');
  if (opportunity.exploitative) reasons.push('exploitative posture violates moral boundary');
  if (opportunity.counterpartyRisk === 'high') reasons.push('high counterparty risk');
  if (opportunity.structuralRisk === 'high') reasons.push('high structural risk');
  if (opportunity.liquidityScore < 50) reasons.push('insufficient liquidity quality');
  if (opportunity.edgeBps < 12) reasons.push('insufficient edge for safe deployment');

  if (f.honeypotProbability >= 0.35) reasons.push('toxicity probability elevated');
  if (f.criticalAuditFindings > 0) reasons.push('critical audit findings present');
  if (f.unresolvedIncidents30d > 0) reasons.push('recent incidents unresolved');
  if (f.adminKeyModel === 'single_key' || f.upgradeability === 'unbounded') {
    reasons.push('governance control surface is too permissive');
  }
  if (f.top10HolderConcentrationPct > 45) reasons.push('holder concentration risk elevated');
  if (f.liquidityLockDays < 30 || f.liquidityLockPercent < 80) reasons.push('liquidity lock posture is weaker than preferred');
  if (risk.marketIntegrityRisk >= 55) reasons.push('market integrity risk elevated');
  if (risk.dependencyContagionRisk >= 50) reasons.push('dependency contagion risk elevated');
  if (risk.attackSurfaceRisk >= 50) reasons.push('attack surface risk elevated');
