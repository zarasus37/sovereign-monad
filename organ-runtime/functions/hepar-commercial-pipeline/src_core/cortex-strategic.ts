...
function normalizeStress(context: CortexStrategicContext): number {
...
function scenarioSet(context: CortexStrategicContext, stress: number): CortexScenario[] {
...
function recommendations(context: CortexStrategicContext, stress: number): CortexRecommendation[] {
  const out: CortexRecommendation[] = [];
...
function causalDrivers(context: CortexStrategicContext, stress: number): string[] {
...
function buildReport(context: CortexStrategicContext): CortexStrategicReport {
...
export function buildCortexStrategicSnapshot(
  contexts: CortexStrategicContext[],
): CortexStrategicRuntimeSnapshot {
  const reports = contexts.map(buildReport);
  const averageStress =
    reports.length === 0 ? 0 : reports.reduce((sum, report) => sum + report.stressIndex, 0) / reports.length;
