...
const DEFAULT_COEFFICIENTS: CardiaAdaptiveCoefficients = {
  returnWeight: 1.2,
...
function riskAdjustedScore(
  ...
  const returnComponent =
    candidate.expectedReturnPct * clamp01(candidate.confidence) * coefficients.returnWeight;
...
function deployableBudgetUsd(state: CardiaAdaptiveState): number {
  if (state.liquidityStress || state.volatilityRegime === 'crash') return 0;
...
function decisionForCandidate(
  ...
  if (score >= coefficients.allocateThreshold && budget > 0) {
    const allocation = Math.min(
      candidate.requiredCapitalUsd,
      budget * clamp01(candidate.confidence),
    );
    return {
      candidateId: candidate.id,
      protocolId: candidate.protocolId,
...
function stressActions(state: CardiaAdaptiveState): string[] {
...
export function buildCardiaAdaptiveSnapshot(
  state: CardiaAdaptiveState,
  candidates: CardiaAllocationCandidate[],
  coefficients?: Partial<CardiaAdaptiveCoefficients>,
): CardiaAdaptiveRuntimeSnapshot {
  const effectiveCoefficients: CardiaAdaptiveCoefficients = {
    ...DEFAULT_COEFFICIENTS,
    ...(coefficients || {}),
  };
  const decisions = candidates.map((candidate) =>
    decisionForCandidate(state, candidate, effectiveCoefficients),
  );
