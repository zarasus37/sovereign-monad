...
function buildOffer(input: RevenueOffer): RevenueOffer {
  return input;
}
...
function annualizeBandUsd(band: RevenueValueBandUsd): { low: number; high: number } {
  const annualLow = band.annualLowUsd ?? ((band.monthlyLowUsd || 0) * 12);
  const annualHigh = band.annualHighUsd ?? ((band.monthlyHighUsd || 0) * 12);
  return {
...
function statusFromScore(score: number): RevenueOffer['status'] {
  if (score >= 70) return 'ready';
  if (score >= 40) return 'developing';
  return 'blocked';
}

function defaultTier3OperationalMaturity(
  input?: Partial<Tier3OperationalMaturity>,
): Tier3OperationalMaturity {
  return {
    automatedReportTemplating: input?.automatedReportTemplating ?? false,
    qualityChecklistSignoff: input?.qualityChecklistSignoff ?? false,
    evidencePackagingAutomation: input?.evidencePackagingAutomation ?? false,
    customerPortalEnabled: input?.customerPortalEnabled ?? false,
    slaEnabled: input?.slaEnabled ?? false,
    targetTurnaroundHours: input?.targetTurnaroundHours ?? 72,
  };
}

function scoreTier1(
  heparCoverage: number,
  heparApprovalRate: number,
  narrativePackages: number,
  exchangeReadyLeads: number,
...
function scoreTier3(
  ...
  exchangeReadyLeads: number,
  reserveHealthy: boolean,
  tier3Operational?: Partial<Tier3OperationalMaturity>,
): { score: number; blockers: string[] } {
  let score = 0;
  const blockers: string[] = [];
...
function buildPortfolioMetrics(
  offers: RevenueOffer[],
): { portfolioMetrics: RevenuePortfolioMetrics; recommendedLaunchSequence: string[] } {
  ...
  const weightedReadinessScore =
    offers.length === 0 ? 0 : Math.round((weightedReadinessNumerator / offers.length) * 100) / 100;
  ...
  const recommendedLaunchSequence = [...offers]
    .sort((left, right) => {
      ...
      const statusWeight: Record<RevenueOffer['status'], number> = {
        ready: 0,
        developing: 1,
        blocked: 2,
      };
...
export function buildRevenueSnapshot(input: {
  ...
  const tier1 = scoreTier1(
    heparCoverage,
    heparApprovalRate,
    narrativePackages,
    exchangeReadyLeads,
    input.cardia.reserveHealthy,
  );
  const tier2 = scoreTier2(
    heparCoverage,
    monetizableBriefs,
    narrativePackages,
    exchangeReadyLeads,
  ...
  const tier3 = scoreTier3(
    heparCoverage,
    monetizableBriefs,
    narrativePackages,
    exchangeReadyLeads,
    input.cardia.reserveHealthy,
    input.tier3Operational,
  );
  const tier4 = scoreTier4(
    heparCoverage,
    heparApprovals,
    monetizableBriefs,
    narrativePackages,
  ...
  const crossTier = scoreCompoundRail(
    heparCoverage,
    monetizableBriefs,
    narrativePackages,
    exchangeReadyLeads,
  ...
  const offers: RevenueOffer[] = [
    ...
    buildOffer({
      ...
      valueBandUsd: {
        annualLowUsd: 10_000_000,
        annualHighUsd: 50_000_000,
      },
      unitEconomics: {
        grossMarginClass: 'high',
  ...
  return {
    implemented: true,
    thesis:
      'The six organs generate sellable intelligence and conversion surfaces; optimization now emphasizes launch order, readiness scoring, and unit-economics discipline before scaling enterprise lanes.',
    offers,
    recommendedLaunchSequence,
    portfolioMetrics,
  };
