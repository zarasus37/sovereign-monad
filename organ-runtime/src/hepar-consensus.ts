...
function severityScore(severity: HeparFindingSeverity): number {
  switch (severity) {
    case 'low':
      return 2;
    case 'medium':
...
function maxSeverity(a: HeparFindingSeverity, b: HeparFindingSeverity): HeparFindingSeverity {
  return severityScore(a) >= severityScore(b) ? a : b;
}

function proofTerm(status: HeparSymbolicStatus): number {
  switch (status) {
    case 'counterexample':
      return 1;
    case 'unknown':
...
function decisionBand(score: number): HeparConsensusDecisionBand {
  if (score <= 19) return 'allow';
  if (score <= 39) return 'guarded_allow';
  if (score <= 59) return 'restricted';
...
function findingKey(finding: HeparAgentFinding): string {
  return finding.vectorId.trim().toLowerCase();
}

type VectorAggregate = {
  vectorId: string;
  title: string;
  severity: HeparFindingSeverity;
  estimatedLossUsd: number;
  agents: Set<string>;
  reproducibleAgents: Set<string>;
};

function summarizeCampaign(campaign: HeparConsensusCampaign): HeparConsensusResult {
  ...
  const meanCoverage =
    totalAgents === 0
      ? 0
      : clamp01(
          campaign.agentRuns.reduce((sum, run) => sum + clamp01(run.coverage), 0) / totalAgents,
        );

  const symbolicVerdicts = new Map<string, HeparSymbolicStatus>();
  for (const verdict of campaign.symbolicVerdicts || []) {
    symbolicVerdicts.set(verdict.vectorId.trim().toLowerCase(), verdict.status);
  }

  const vectors = new Map<string, VectorAggregate>();
    ...
    for (const finding of run.findings) {
      ...
      if (!aggregate) {
        aggregate = {
          ...
          reproducibleAgents: new Set<string>(),
        };
  ...
  const vectorResults: HeparConsensusVector[] = [];
...
export function buildHeparConsensusSnapshot(
  campaigns: HeparConsensusCampaign[],
): HeparConsensusRuntimeSnapshot {
