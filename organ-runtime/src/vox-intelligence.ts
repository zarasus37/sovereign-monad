...
function truthStatus(input: VoxNarrativeInput): VoxNarrativeTruthStatus {
  if (input.contradictionCount > 0) return 'conflicted';
  if (input.facts.length > 0 && input.proofRefs.length > 0) return 'verified';
  if (input.facts.length > 0) return 'incomplete';
  return 'conflicted';
}

function channelFor(audience: VoxAudiencePackage['audience']): string {
  switch (audience) {
    case 'operators':
...
function summaryForAudience(input: VoxNarrativeInput, audience: VoxAudiencePackage['audience']): string {
...
function buildAudiencePackages(input: VoxNarrativeInput): VoxAudiencePackage[] {
  const status = truthStatus(input);
  const warnings: string[] = [];
  if (status === 'incomplete') warnings.push('evidence references are missing for one or more claims');
  if (status === 'conflicted') warnings.push('narrative contains unresolved contradictions');
...
export function buildVoxNarrativeIntelligenceSnapshot(
  inputs: VoxNarrativeInput[],
): VoxNarrativeIntelligenceSnapshot {
