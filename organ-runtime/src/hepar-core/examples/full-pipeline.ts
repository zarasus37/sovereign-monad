...
import { createDefaultHeparOrchestrator } from '../HeparOrchestrator';
...
async function main() {
  ...
  const addressesToProbe = [
    '0xE592427A0AEce92De3Edee1F18E0157C05861564', // SwapRouter02
  ...
  try {
    ...
    const pipelineResult = await orchestrator.executeFullPipeline(protocolId, addressesToProbe);
    ...
    if (pipelineResult.stageD) {
      ...
      const voteCounts: Record<string, number> = { BLOCK: 0, WARN: 0, ALLOW: 0, INVESTIGATE: 0 };
      pipelineResult.stageD.votes.forEach(v => {
        voteCounts[v.decision]++;
      });
      Object.entries(voteCounts).forEach(([decision, count]) => {
        if (count > 0) console.log(`  ${decision}: ${count} votes`);
      });
    }
