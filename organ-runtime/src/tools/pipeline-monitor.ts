import { getContainer } from '../hepar/commercial/cosmos-config';

async function safeFetch(containerName: string) {
    try {
...
async function runMonitor() {
    ...
    leads.forEach(l => {
        if (l.priority === 'IMMEDIATE') immediate++;
        else if (l.priority === 'URGENT') immediate++;
        else if (l.priority === 'STANDARD') standard++;
        else longitudinal++;
        ...
        if (l.timestamp) {
            ...
            if (now - new Date(l.timestamp).getTime() <= 6 * 60 * 60 * 1000) recentLeads++;
        }
    });
    ...
    opps.forEach(o => {
        if (o.classification === 'ALLOW') allow++;
        else if (o.classification === 'NEAR_MISS') nearMiss++;
        else if (o.classification === 'RESTRICTED') restricted++;
        else if (o.classification === 'HARDBLOCK') hardblock++;

        if (o.timestamp) {
            if (latestOppTimestamp === "N/A" || new Date(o.timestamp).getTime() > new Date(latestOppTimestamp).getTime()) {
                latestOppTimestamp = o.timestamp;
            }
            if (now - new Date(o.timestamp).getTime() <= 6 * 60 * 60 * 1000) recentOpps++;
        }
    });
    ...
    props.forEach(p => {
        if (p.cortexEnhanced) cortex++;
        if (p.synapseCoordinated) synapse++;
        if (p.pneumaEnhanced) pneuma++;
        if (p.voxEnhanced) vox++;

        if (p.distributionTier === 'PUBLIC') publicTier++;
        else if (p.distributionTier === 'NDA') ndaTier++;
        else if (p.distributionTier === 'INTERNAL') internalTier++;
        ...
        if (p.timestamp) {
            ...
            if (now - new Date(p.timestamp).getTime() <= 6 * 60 * 60 * 1000) recentProps++;
        }
    });
    ...
    outreachList.forEach(o => {
        ...
        else if (o.status === 'RESPONDED') responded++;
        else if (o.status === 'CONVERTED') converted++;

        if (o.status === 'SENT' && o.followUp1Sent === false) pending72h++;
        if (o.status === 'SENT' && o.followUp2Sent === false) pending7d++;
        if (o.status === 'RESPONDED') humanResponses++;
        ...
        if (o.timestamp) {
            ...
            if (now - new Date(o.timestamp).getTime() <= 24 * 60 * 60 * 1000) recentOutreach++;
        }
    });
    ...
    let activeClients = 0;
    clients.forEach(c => {
        if (c.status === 'ACTIVE_MONITORING') activeClients++;
    });
