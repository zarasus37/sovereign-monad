...
export function buildFirstMandateSnapshot(input: {
  title: string;
  ...
  if (input.synapse.sampleSignalCount === 0) gateChecks.push('Synapse needs at least one signal');
  if (input.hepar.approvedCount === 0) gateChecks.push('Hepar needs at least one bounded approved opportunity');
  if (input.cortex.sourceCount === 0) gateChecks.push('Cortex needs research input');
  if (input.vox.requestCount === 0) gateChecks.push('Vox needs at least one narrative request');
  if (input.pneuma.acceptedCount === 0) gateChecks.push('Pneuma needs at least one accepted exchange candidate');
  if (input.immune.barrierTriggerCount > 0) gateChecks.push('Immune layer has active barrier triggers requiring review');
