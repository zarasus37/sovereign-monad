...
exports.buildVoxSnapshot = exports.packageNarrative = void 0;
function channelFor(format, audience) {
    if (format === 'newsletter')
        return 'newsletter';
    if (format === 'memo' || audience === 'operators')
        return 'operator-memo';
    if (audience === 'buyers' || audience === 'partners')
        return 'direct-outreach';
    if (format === 'briefing')
        return 'briefing-room';
    return 'social-thread';
}
function callToActionFor(audience) {
    switch (audience) {
        case 'operators':
...
function packageNarrative(request, briefs) {
...
exports.packageNarrative = packageNarrative;
function buildVoxSnapshot(requests, briefs) {
    return {
        implemented: true,
...
exports.buildVoxSnapshot = buildVoxSnapshot;
