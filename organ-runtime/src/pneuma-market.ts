...
function counterpartyOk(
  ...
  if (signal.solvencyRisk === 'high') return false;
  if (signal.settlementReliability < policy.minSettlementReliability) return false;
  if (signal.complianceBlocked) return false;
  return true;
}

function executionCostBps(
...
export function buildPneumaMarketSnapshot(
  orders: PneumaOrderIntent[],
  quotes: PneumaVenueQuote[],
  counterparties: PneumaCounterpartySignal[],
  policy?: Partial<PneumaExecutionPolicy>,
): PneumaMarketIntelligenceSnapshot {
  const effectivePolicy: PneumaExecutionPolicy = {
    ...DEFAULT_POLICY,
    ...(policy || {}),
  };
  const decisions = orders.map((order) =>
    pickBestQuote(order, quotes, counterparties, effectivePolicy),
  );
  const acceptedCount = decisions.filter((decision) => decision.accepted).length;
  const averageCostBps =
    acceptedCount === 0
      ? 0
      : decisions
