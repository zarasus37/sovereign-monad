...
import { ApiStatus, GameFiSourceConfig, SourceHealthSnapshot } from './types';
import { buildSnapshotFromConfig } from './utils/sourceState';

const API_URL = import.meta.env.VITE_GAMEFI_API_URL as string | undefined;
const CONFIG_URL =
  (import.meta.env.VITE_GAMEFI_SOURCE_CONFIG_URL as string | undefined) ||
  '/gamefi-source.sample.json';

function useGameFiHealth(): {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (API_URL) {
        setMode('api');
        try {
          const res = await fetch(`${API_URL}/gamefi/source-health`);
          if (!res.ok) {
            throw new Error(`gamefi-control API returned ${res.status}`);
          }
      ...
      if (CONFIG_URL) {
        ...
        try {
          ...
          if (!res.ok) {
            throw new Error(`gamefi config returned ${res.status}`);
          }

          const data = (await res.json()) as GameFiSourceConfig;
          if (!cancelled) {
...
export default function App() {
  const { snapshot, status, mode } = useGameFiHealth();
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontWeight: 700, letterSpacing: '0.1em', fontSize: '1rem' }}>
            HIGH-FIDELITY DEFI GAMING CONTROL
          </span>
      ...
      <main style={{ padding: '1.5rem', maxWidth: '900px' }}>
        <section style={{ marginBottom: '2rem' }}>
          ...
          {status === 'error' && (
            ...
            >
              Failed to load gaming rail source status from the configured data source.
            </div>
          )}
            ...
            <div style={{ marginBottom: '0.4rem' }}>
              ...
              <span style={{ opacity: 0.7 }}>
                {snapshot?.sources.bootstrap.address ||
                  'wallet designated locally - materialize gamefi-source.json after on-chain registration'}
              </span>
            </div>
            <div>
