/**
 * Base WebSocket RPC client using ethers.js v6.
...
export class BaseRpcClient implements RpcClient {
  ...
  constructor(wsUrl: string) {
    ...
    this.endpoints = [];
    for (const url of [wsUrl, ...BASE_MAINNET_WS_FALLBACKS]) {
  ...
  async disconnect(): Promise<void> {
    if (this.provider) {
      logger.info('Disconnecting from Base WebSocket');
...
export function createRpcClient(wsUrl: string): RpcClient {
  return new BaseRpcClient(wsUrl);
}
