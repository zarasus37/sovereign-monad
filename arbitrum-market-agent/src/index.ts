/**
 * Entry point for arbitrum-market-agent
 * Handles graceful shutdown on SIGINT/SIGTERM
 */

import { createAgent, ArbitrumMarketAgent } from './agent';
import { createLogger } from './utils/logger';

const logger = createLogger('main');

let agent: ArbitrumMarketAgent | null = null;

async function main(): Promise<void> {
  logger.info('Starting arbitrum-market-agent');
...
process.on('unhandledRejection', (reason: any) => {
  ...
  // Don't crash on subscription errors — the RPC adapter handles failover
